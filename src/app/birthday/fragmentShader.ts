export const fragmentShader = `
    ...
    float text(vec2 uv)
    {
      ...
      print_pos = floor(center - vec2(STRWIDTH(8.0), STRHEIGHT(-2.0)));
      ...
      print_pos = floor(center - vec2(STRWIDTH(7.0), STRHEIGHT(-0.0)));

      float mod3 = mod(second, 3.0);
      float mod2 = mod(second, 2.0);

      TEXT_MODE = INVERT;

      col += char(mod3 == 2.0 ? ch_ast : ch_spc, uv);
      col += char(mod3 == 1.0 ? ch_ast : ch_spc, uv);
      col += char(mod3 == 0.0 ? ch_ast : ch_spc, uv);
      col += char(ch_spc, uv);
      col += char(ch_O, uv);
      col += char(ch_v, uv);
      col += char(ch_e, uv);
      ...
      print_pos = floor(center - vec2(STRWIDTH(3.0), STRHEIGHT(2.0)));
      ...
      print_pos = floor(center - vec2(STRWIDTH(24.0), STRHEIGHT(4.0)));
      ...
      col += char(ch_s, uv);
      col += char(ch_spc, uv);
      col += char((mod2 == 1.0) ? ch_crs : ch_spc, uv);
      col += char((mod2 == 0.0) ? ch_crs : ch_spc, uv);
      col += char((mod2 == 1.0) ? ch_crs : ch_spc, uv);
      col += char((mod2 == 0.0) ? ch_crs : ch_spc, uv);
    ...
    void mainImage( out vec4 fragColor, in vec2 fragCoord )
    {
        ...
        vec2 duv = floor(fragCoord.xy / (1. + sin(iTime) / 5.));

        duv += (cos(iTime) + sin(iTime)) * 20.;
